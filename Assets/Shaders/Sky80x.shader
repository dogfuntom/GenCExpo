...
//              for(int i=0; i<int(kSamples); i++) // Loop removed because we kept hitting SM2.0 temp variable limits. Doesn't affect the image too much.
        ...
        // 0 for no sun, 1 for unchanged sun color
        half3 sampleSunTexture(half3 lightPos, half3 ray)
        {
            half3 lightRight = normalize(half3(-lightPos.z, 0, lightPos.x));
            half3 lightUp = cross(normalize(lightPos), lightRight);
            ...
            //half hor = length(delta.xz) * sign(delta.x) * sign(delta.z);
            //half ver = delta.y;
            half hor = dot(delta, lightRight);
            half ver = dot(delta, lightUp);

            half2 signedUV = half2(hor, ver) / _SunSize;
            half2 uv = signedUV * 0.5 + 0.5;

            half4 col = tex2D(_MainTex, uv);
            return col.xyz * col.a;
            //return half3(uv.r, uv.g, 0);
        }
        ...
        // if y < 0 [eyeRay.y > 0] - sky
            ...
            if(y < 0.0)
            {
                col += IN.sunColor * calcSunAttenuation(_WorldSpaceLightPos0.xyz, -ray)
                    * sampleSunTexture(_WorldSpaceLightPos0.xyz, -ray);
            }
        #endif
        ...
        // ray.x is [-1, 1] x coordinate, except ray.x=0 is when scene's x=1, so the ray is towards us.
        // ray.yz are the same deal.
        // #if SKYBOX_SUNDISK != SKYBOX_SUNDISK_NONE
            // return half4(0.0, ray.y * 0.5 + 0.5, 0.0, 1.0);
        // #endif
